...
def product_in_order_post_save(sender, instance, created, **kwargs):
    # не сохранять для модели с которой работаем(будут зацикливание)
    order = instance.order
    all_order_in_order = ProductInOrder.objects.filter(order=order, is_active=True)
    order_total_price = 0
    for item in all_order_in_order:
        order_total_price += item.total_price
    instance.order.total_price = order_total_price
    instance.order.save(force_update=True)


post_save.connect(product_in_order_post_save, sender=ProductInOrder)#вызывается после метода save
#
